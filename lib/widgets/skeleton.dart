...
class Skeleton extends StatefulWidget {
  ...
  @override
  State<Skeleton> createState() => _FLSkeletonState();
}

class _FLSkeletonState extends State<Skeleton> with SingleTickerProviderStateMixin {
  ...
  Tween<double> _genTween() {
    return (widget.type == FLSkeletonAnimationType.shimmer) ? Tween<double>(begin: -1.0, end: 2.0) : Tween<double>(begin: widget.width, end: widget.stretchWidth);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        ...
        final Gradient gradient = widget.type == FLSkeletonAnimationType.shimmer ? LinearGradient(begin: isRtl ? Alignment.centerRight : Alignment.centerLeft, end: isRtl ? Alignment.centerLeft : Alignment.centerRight, colors: [color, shimmerColor.withAlpha(200), color], stops: [_animation.value - 0.4, _animation.value, _animation.value + 0.4]) : null;
        final double width = widget.type == FLSkeletonAnimationType.shimmer ? widget.width : _animation.value;
        final decColor = widget.type == FLSkeletonAnimationType.stretch ? color : null;
