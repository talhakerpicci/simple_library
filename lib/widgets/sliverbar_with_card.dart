...
class _CardSliverAppBarState extends State<CardSliverAppBar> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _fadeTransition = Tween(begin: 1.0, end: 0.0).animate(CurvedAnimation(parent: _animationController, curve: Interval(0.4, 1.0, curve: Curves.easeIn)))
      ..addListener(() {
        setState(() {});
      });
    if (widget.card != null) {
      _rotateCard = Tween(begin: 0.0, end: 0.4).animate(CurvedAnimation(curve: Curves.linear, parent: _animationController))
        ..addListener(() {
          setState(() {});
        });
    }
  ...
  get _ratingBar => widget.ratingBar;
  ...
  Widget build(BuildContext context) {
    ...
    if (_scale >= 0.5) {
      ...
      stackOrder.add(_ratingBarConstructor());
      if (_card != null) stackOrder.add(_cardConstructor());
      if (_action != null) stackOrder.add(_actionConstructor());
      if (_backButton != null && _backButton) stackOrder.add(_backButtonConstructor());
    } else {
      stackOrder.add(_backgroundConstructor());
      if (_card != null) stackOrder.add(_cardConstructor());
      stackOrder.add(_bodyContainer());
      ...
      child: Column(
        children: <Widget>[
          IconButton(
            ...
            color: _animatedBackButtonColors != null ? _animatedBackButtonColors.evaluate(AlwaysStoppedAnimation(_animationController.value)) : Colors.white,
            iconSize: 25,
  ...
  double _getCardTopMargin() {
    final double value = _scale <= 0.5 ? widget.height - ((widget.appBarHeight * 3.6) * _scale) : widget.height - (widget.appBarHeight * 1.8);
    return value;
  }
    ...
    return Positioned(
      ...
      child: ClipPath(
        ...
        child: Container(
          ...
          padding: EdgeInsets.only(left: _scale >= 0.12 ? 40 + ((MediaQuery.of(context).size.width / 4) * _scale) : 50),
          width: MediaQuery.of(context).size.width,
          color: Colors.white,
  ...
  Widget _ratingBarConstructor() {
    return Positioned(
      key: Key("widget_ratingBar"),
      top: _scale == 0.0 ? _offset : widget.height - widget.appBarHeight + 68,
      left: 0,
    ...
    if (_titleDescription != null) {
      ...
      var titleDescriptionContainer = Opacity(
        ...
        child: FittedBox(
          ...
          child: Container(
            alignment: Alignment.centerLeft,
            padding: EdgeInsets.only(top: (25 * _scale), right: 10),
